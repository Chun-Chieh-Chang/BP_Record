...
- 將 X 軸與 Y 軸刻度字體大小調整為 13px，平衡在移動端與電腦端顯示時的數字清晰度與空間佔用。
...
- **圖表設定 (`index.html`)**：
  - 更新 Chart.js 配置中的 `options.scales.x` 和 `options.scales.y` 屬性，加入 `ticks.font` 設定，並指定 `size` 為 `13`。
- **快取更新 (`assets/sw.js`)**：
  - 將 Service Worker 的快取版本升級為 `bp-nexus-v9`，強制行動端瀏覽器即時套用最新優化。

### 🔍 Check
- [x] 軸刻度字體是否更清晰易讀？（是，13px 字體提供了完美的版面協調度與數字易讀性）
